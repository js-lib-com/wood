...
import com.jslib.wood.lang.ReaderInputStream;
 ...
 * type; so to fully refer a variable one need to know both type and name. See {@link Reference.Type} for recognized types and
...
public class Variables {
     ...
     * <p>
     * Variables definition file is a standard XML with root element one of the {@link Reference.Type#variables()} values. It is
     * legal that provided <code>file</code> parameter to not point to a variables definition file. If this is the case SAX
     * parsing is aborted eagerly by {@link SAXHandler}; this way SAX parsing is also used to detect if file is valid variables'
     * definition.
     *
    ...
    public String get(String language, Reference reference, FilePath source, IReferenceHandler handler) throws WoodException {
        ...
        if (levelTrace.contains(trace)) {
            ...
            for (String s : levelTrace) {
                builder.append(StringsUtil.concat("\t- ", s, "\n"));
            }
     ...
     * <p>
     * See below sample file for expected format. {@link Reference.Type} is the root of the XML document. A resources file can
     ...
     * children - e.g. {@link Reference.Type#TEXT}, they are processed like an XML fragment and copied with element start/end tags
     ...
     * variable definition file is an XML file with root element one of {@link Reference.Type#variables()} values.
     *
     * @author Iulian Rotaru
     * @since 1.0
     */
     ...
     * {@link Reference.Type} encoded by root - see {@link #instance(Reference.Type)}. Builder instance is reused for all variable
     * elements from file; there is {@link #reset()} method that prepare instance for new value. Once variable element
     * discovered, value builder helps to collect variable value from XML stream via {@link #addValue(char[], int, int)} method.
     * <p>
     * This class deals with plain string only. There is specialized value builder for formatted text, see
     * {@link TextValueBuilder}. Also, this base class defines hooks methods for subclasses benefit: {@link #startTag(String)} and {@link #endTag(String)}.
     *
     * @author Iulian Rotaru
     * @since 1.0
     */
    private static class ValueBuilder {
        ...
        public static ValueBuilder instance(Reference.Type referenceType) {
            if (Objects.requireNonNull(referenceType) == Reference.Type.TEXT) {
    ...
    /**
     * Value builder for {@link Reference.Type#TEXT} variables. This builder collect formatted text that is in essence an HTML
