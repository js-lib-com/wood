...
public class SourceReader extends Reader {
    ...
    public SourceReader(Reader sourceReader, FilePath sourceFile, IReferenceHandler referenceHandler) {
        super();
        assert sourceReader != null : "Source reader argument is null";
        assert sourceFile != null : "Source file argument is null";
        assert sourceFile.isSynthetic() || sourceFile.exists() : "Source file argument does not exist";
        assert referenceHandler != null : "Reference handler argument is null";
    ...
    public SourceReader(FilePath sourceFile, LayoutParameters layoutParameters, IReferenceHandler referenceHandler) {
        this(reader(sourceFile), sourceFile, referenceHandler);
        assert sourceFile.isLayout(): "Source file argument is not a widget or template layout";
    ...
    /**
     * This method is not needed for source reader logic but is required by reader interface. It just fills the buffer, reading
     * char by char.
     *
                ...
                if (reference != null) {
                    if (reference.getType() == Reference.Type.PARAM) {
                        ...
                        if (sourceFile.isXmlLike()) {
                            value = StringsUtil.escapeXML(value);
                        }
                    }
                } else {
    ...
    private static class MetaBuilder {
        ...
        public boolean add(int c) {
            ...
            if (ESCAPE.contentEquals(builder)) {
                escape = true;
            }
            return true;
        }
