...
public class FilePath implements Iterable<FilePath> {
    ...
    static final Pattern DIRECTORY_PATTERN = Pattern.compile("^" + //
    ...
    public FilePath(Project project, String value) {
        assert project != null : "Project argument is null";
        assert value != null : "Path value argument is null";

        this.project = project;
        this.pathSegments = new ArrayList<>(StringsUtil.split(value, FilePath.SEPARATOR_CHAR));
        ...
        if (!directory) {
            ...
            this.fileName = StringsUtil.concat(this.basename, '.', extension);
     ...
     *
     * @return MIME type.
     * @throws IOException if underlying JVM implementation fails to probe content type.
     */
    ...
    /**
     * Test if this directory path is an HTML component.
     *
     * @return true if this directory path is an HTML component.
     */
    ...
    /**
     * Test if this file is a component descriptor. A component descriptor is an XML file that has base name the same as
    ...
    public boolean isXml(String... roots) {
        ...
        try (BufferedReader reader = new BufferedReader(getReader())) {
            ...
            for (String root : roots) {
                if (line.startsWith(StringsUtil.concat('<', root, '>'))) {
                    return true;
                }
            }
            return false;
     ...
     */
    public FilePath getSubDirectoryPath(String path) {
    ...
    /**
     * Iterate over all this directory direct children, both files and subdirectories. Please note that iteration order is not
    ...
    public FilePath cloneTo(FileType type) {
        return new FilePath(project, FilesUtil.replaceExtension(value(), type.extension()));
    }
    ...
    public String load() throws IOException {
        return StringsUtil.load(getReader());
    }
     ...
     * enter subdirectories. In fact directories are ignored. Hidden files are also ignored; a hidden file has a name that
    ...
    private class FilesIterator implements Iterator<FilePath> {
        ...
        public FilesIterator(File[] files) {
            assert files != null : "Files list argument is null";
            this.files = files;
            this.index = -1;
        }
